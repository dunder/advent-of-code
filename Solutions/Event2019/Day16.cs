    ...
    public class Day16
    {
        ...
        private static string Run(List<int> input, int phases)
        {
            for (int phase = 1; phase <= phases; phase++)
            {
                ...
                for (int outputPosition = 0; outputPosition < input.Count; outputPosition++)
                {
                    var positionPattern = pattern.SelectMany(d => Enumerable.Repeat(d, outputPosition + 1)).Skip(1).ToList();
                    ...
                    output.Add(Math.Abs(positionOutput.Sum()));
                }
